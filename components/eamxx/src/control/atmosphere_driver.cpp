...
void AtmosphereDriver::restart_model ()
{
  ...
  for (auto& gn : m_grids_manager->get_grid_names()) {
    ...
    read_fields_from_file (fnames,m_grids_manager->get_grid(gn),filename,m_current_ts);
  }
...
void AtmosphereDriver::set_initial_conditions ()
{
  ...
  if (ic_pl.isParameter("Filename")) {
    ...
    for (const auto& it : m_grids_manager->get_repo()) {
      ...
      if (not m_iop_data_manager) {
        read_fields_from_file (ic_fields_names[grid_name],grid,file_name,m_current_ts);
      } else {
  ...
  if (num_perturb_fields > 0) {
    ...
    for (size_t f=0; f<perturbed_fields.size(); ++f) {
      ...
      EKAT_REQUIRE_MSG(ekat::contains(fields_inited[gll_grid_name], fname),
                       ...
                       const util::TimeStamp& t0)
{
  ...
  for (size_t i=0; i<field_names_nc.size(); ++i) {
    ...
    fields.push_back(m_field_mgr->get_field(eamxx_name, gn).alias(nc_name));
  }
