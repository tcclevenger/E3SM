       ...
       const std::set<std::string>& grid_names)
{
  ...
  if (grid_names.size()==1) {
    auto output = std::make_shared<output_type>(m_io_comm,m_params,field_mgr,*fields_pl.sublists_names_cbegin());
  ...
  if (m_is_model_restart_output) {
    ...
    for (const auto& gname : grid_names) {
      ...
      if (field_mgr->has_group("RESTART", gname)) {
        auto restart_group = field_mgr->get_groups_info().at(gname).at("RESTART");
