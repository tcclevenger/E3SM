                               ...
                               const int time_index)
{
  ...
  EKAT_REQUIRE_MSG(field_mgr->get_grids_manager()->has_grid(grid_name),
                   "Error! Field manger does not contain the requested grid: " + grid_name + "\n");
  ...
  for (size_t i=0; i<field_names_nc.size(); ++i) {
    ...
    const auto& fm_field = eamxx_name!=nc_name
                           ?
                           field_mgr->get_field(eamxx_name, grid_name).alias(nc_name)
                           :
                           field_mgr->get_field(eamxx_name, grid_name);
  ...
  for (size_t i=0; i<io_fields.size(); ++i) {
    ...
    auto& fm_field = field_mgr->get_field(fname, grid_name);
